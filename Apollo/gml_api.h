...
struct gml_Func_t {
	...
	YYFunc method_get_index = nullptr;
	YYFunc is_method = nullptr;
};
...
namespace GML {
	...
	template<size_t size> void script_execute_def_for(RValue& result, RValue(&args)[size], lua_State* L) {
		Apollo::clearNextError();
		script_execute_def(result, args, (int)size);
		if (Apollo::hasNextError()) {
			result.free();
			for (auto i = 0u; i < size; i++) args[i].free();
			Apollo::throwNextError(L);
		}
	}
